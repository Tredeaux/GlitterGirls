...
if ($id) {
    try {
        ...
        if ($result["certified"] == 1) {
            try {
                $stmt = $conn->prepare("UPDATE `biofefqs_maindata`.`userdata` SET `certified` = 0 WHERE `id` = :a1"); 
                $stmt->execute([":a1" => $id]);
            } catch(PDOException $e) {
                //echo "Error: " . $e->getMessage();
            }
        } else if ($result["certified"] == 0) {
            try {
                $stmt = $conn->prepare("UPDATE `biofefqs_maindata`.`userdata` SET `certified` = 1 WHERE `id` = :a1"); 
                $stmt->execute([":a1" => $id]);
            } catch(PDOException $e) {
                //echo "Error: " . $e->getMessage();
            }
